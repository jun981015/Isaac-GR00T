...
conda env name:   gr00t-train
conda env path:   /home/junhyeong/miniconda3/envs/gr00t-train
```
...
```bash
conda env remove -n gr00t-train
ENV_NAME=gr00t-train envs/install_groot_smoke_conda.sh
```
...
```bash
conda run -n gr00t-train python ...
```

The install script writes conda activation hooks:

```text
$CONDA_PREFIX/etc/conda/activate.d/gr00t-train.sh
$CONDA_PREFIX/etc/conda/deactivate.d/gr00t-train.sh
```
...
```bash
nvidia-smi
conda run -n gr00t-train python -c "import torch; print(torch.__version__, torch.version.cuda, torch.cuda.is_available())"
```
...
```bash
conda run -n gr00t-train python scripts/inference_service.py --help
```
...
Fix: activate the env so the hook runs, or inspect $CONDA_PREFIX/etc/conda/activate.d/gr00t-train.sh.
```
...
```text
conda env exists: conda info --envs | grep gr00t-train
