...
This document is the operational handoff for running Isaac-GR00T RoboCasa finetuning with the conda env `groot-smoke`, replacing the previous Docker-based GR00T training workflow.
...
2. Build or activate conda env groot-smoke.
...
```bash
echo "$CONDA_PREFIX"
ls "$CONDA_PREFIX/etc/conda/activate.d/groot-smoke.sh"
```
...
```bash
conda run -n groot-smoke wandb status
```
