...
if conda env list | awk '{print $1}' | grep -Fxq "${ENV_NAME}"; then
  echo "Conda env already exists, continuing install into: ${ENV_NAME}"
else
...
CLEAN_SRC="$(mktemp -d "/tmp/${ENV_NAME}-src.XXXXXX")"
cleanup_clean_src() {
  echo "Leaving temporary clean source copy at: ${CLEAN_SRC}"
}
...
cat > "${ACTIVATE_DIR}/${ENV_NAME}.sh" <<EOF
...
EOF
cat > "${DEACTIVATE_DIR}/${ENV_NAME}.sh" <<'EOF'
