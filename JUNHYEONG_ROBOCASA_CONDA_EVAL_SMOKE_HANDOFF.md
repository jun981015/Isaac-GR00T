...
```text
groot-smoke policy server -> ZMQ -> robocasa-eval simulator client
```

The `groot-smoke` env owns GR00T checkpoint loading and torch/CUDA inference.
...
A conda policy server was launched from `groot-smoke` with:
...
conda run --no-capture-output -n groot-smoke \
...
```bash
nohup conda run --no-capture-output -n groot-smoke python scripts/inference_service.py ...
```
