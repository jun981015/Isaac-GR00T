...
if [[ -f "${CHECKPOINT_DIR}/compact_summary.json" ]]; then
  if [[ ! -d "${HF_CACHE_DIR}" ]]; then
    echo "Compact checkpoint requires HF_CACHE_DIR to exist: ${HF_CACHE_DIR}" >&2
    exit 1
  fi
  while IFS= read -r shard_link; do
    if [[ ! -e "${shard_link}" ]]; then
      echo "Compact checkpoint symlink target is missing: ${shard_link}" >&2
      exit 1
    fi
  done < <(find "${CHECKPOINT_DIR}" -maxdepth 1 -type l -name 'base-*.safetensors' -print)
fi
...
docker run -d \
  ...
  -v "${HF_CACHE_DIR}:${HF_CACHE_DIR}:ro" \
