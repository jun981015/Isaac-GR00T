...
gpus=(1 2)
ports=(8041 8042)
tasks=(PnPCounterToStove PnPMicrowaveToCounter)
...
launch_bench() {
  ...
  local port="$3"
  local task="$4"
  local cname="isaac-gr00t-robocasa-bench-${label}-ckpt20000-${task}-obs128"
...
wait_bench() {
  ...
  docker logs "${cname}" > "${output_dir}/docker_${task}_obs128.log" 2>&1 || true
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] pm8/pm16 stove+microwave obs128 eval complete"
