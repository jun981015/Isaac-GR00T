...
from scripts.robocasa_rollout_hdf5 import RolloutRecorder, sim_state
...
def rollout_worker_episode(
    ...
    try:
        ...
        obs = env.reset()
        recorder = (
            RolloutRecorder(env, episode["ep_meta"], include_images=args.hdf5_include_images)
            if args.save_hdf5_rollout
            else None
        )
        ...
        while env_steps < args.max_episode_steps:
            ...
            for action_idx in range(args.n_action_steps):
                ...
                obs_before = obs
                state_before = sim_state(env) if recorder is not None else None
    ...
    hdf5_path = None
    if recorder is not None:
        hdf5_dir = Path(args.hdf5_dir) if args.hdf5_dir else task_dir / "hdf5"
        hdf5_path = hdf5_dir / f"ep{episode_idx:03d}_seed{config.seed}_outcome{int(success)}.hdf5"
        recorder.write(
            hdf5_path,
...
def run_eval(args: argparse.Namespace) -> None:
    ...
    write_json(
        task_dir / "run_config.json",
        {
            ...
            "save_hdf5_rollout": args.save_hdf5_rollout,
            "hdf5_dir": args.hdf5_dir,
            "hdf5_include_images": args.hdf5_include_images,
            "policy_checkpoint": args.policy_checkpoint,
        },
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--save_hdf5_rollout", action="store_true")
    parser.add_argument("--hdf5_dir", default=None)
    parser.add_argument("--hdf5_include_images", action="store_true")
