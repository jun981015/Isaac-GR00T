...
SAVE_HDF5_ROLLOUT="${SAVE_HDF5_ROLLOUT:-0}"
HDF5_DIR="${HDF5_DIR:-}"
HDF5_INCLUDE_IMAGES="${HDF5_INCLUDE_IMAGES:-0}"
...
if [[ "${SAVE_HDF5_ROLLOUT}" == "1" ]]; then
  EXTRA_ARGS+=(--save_hdf5_rollout)
fi
if [[ -n "${HDF5_DIR}" ]]; then
  EXTRA_ARGS+=(--hdf5_dir "${HDF5_DIR}")
fi
if [[ "${HDF5_INCLUDE_IMAGES}" == "1" ]]; then
  EXTRA_ARGS+=(--hdf5_include_images)
fi
if [[ -n "${POLICY_CHECKPOINT}" ]]; then
  EXTRA_ARGS+=(--policy_checkpoint "${POLICY_CHECKPOINT}")
fi
