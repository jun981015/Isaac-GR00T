...
TASK="${TASK:?set TASK, e.g. PnPMicrowaveToCounter}"
...
run_policy_task() {
  ...
  local gpu="$3"
  local port="$4"
  local server_name="isaac-gr00t-robocasa-zmq-${RUN_ID}-${policy_name}-${TASK}"
  local schedule_path="${SCHEDULE_DIR}/seed1/${TASK}_${N_EPISODES}eps.json"

  test -s "${schedule_path}"
  log "start server policy=${policy_name} task=${TASK} gpu=${gpu} port=${port}"
  CHECKPOINT_DIR="${checkpoint_dir}" \
  GPU_DEVICE="${gpu}" \
  PORT="${port}" \
  CONTAINER_NAME="${server_name}" \
  OUTPUT_DIR="${OUTPUT_ROOT}/${policy_name}/_server_${TASK}" \
  BOOTSTRAP_DEPS=0 \
  REPLACE=1 \
  ...
  for action_seed in "${ACTION_SEEDS[@]}"; do
    ...
    log "eval policy=${policy_name} task=${TASK} action_seed=${action_seed}"
    ...
    if [[ "${code}" != "0" ]]; then
      log "eval failed policy=${policy_name} task=${TASK} action_seed=${action_seed} code=${code}"
  ...
  docker stop "${server_name}" >/dev/null || true
  log "done policy=${policy_name} task=${TASK}"
}
