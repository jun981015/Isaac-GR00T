...
Use these limits for each conda eval or eval container when multiple RoboCasa
evals run concurrently. This prevents CPU oversubscription. The expected speed
loss is small because the bottleneck is usually MuJoCo/env stepping, rendering,
and ZMQ policy communication rather than BLAS/OpenMP-heavy work. Run concurrent
evals in bounded batches, around 6 at a time, instead of launching an unbounded
number. The current atomic eval was run with this policy and completed normally.
...
For conda direct eval, set `MUJOCO_EGL_DEVICE_ID` to the same physical GPU id
used in `CUDA_VISIBLE_DEVICES`, for example `CUDA_VISIBLE_DEVICES=2` with
`MUJOCO_EGL_DEVICE_ID=2`. The robosuite binding asserts that
`MUJOCO_EGL_DEVICE_ID` appears in `CUDA_VISIBLE_DEVICES`.

For Docker/container eval, `--gpus device=<physical_eval_gpu>` masks the
container and the wrapper sets internal `CUDA_VISIBLE_DEVICES=0`, so
`MUJOCO_EGL_DEVICE_ID=0` is correct inside the container. In both paths, the
goal is to prevent MuJoCo/EGL rendering from accidentally landing on physical
GPU0 during concurrent evals.
